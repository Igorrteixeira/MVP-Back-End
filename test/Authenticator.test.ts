...
import { AutheticatorMock,AuthenticationData } from "./mocks/AuthenticatorMock"


describe("Teste Authenticator",()=>{
    test("Deve retornar sucesso se token for gerado", async () => {
        const authenticator = new AutheticatorMock().generateToken({id:"ddd",role:ROLE.VENDEDOR})
    ...
    test("Deve retornar sucesso se encontrar token valido", async () => {
        ...
        const authenticator = new AutheticatorMock().getTokenData("token-mock-vendedor")
